...
TIMELINE_DATA = [
    ...
    (9,  "Technical Blog Posts",
     "Wrote three technical blog posts covering OOP, recursion, and event-driven programming, each with an embedded demo video.",
...
# The shared demo video — served from assets/video/
...
def build_video_player(color):
    """
    Builds a styled native video player using ft.Video.
    The video file must be at:  assets/video/video_2026-06-15_14-52-22.mp4
    Flet's web server serves assets/ as static files, so the src path is
    relative to the assets root — no leading slash needed.
    """
    try:
        # ft.Video is available in Flet >= 0.21
        player = ft.Video(
            playlist=[ft.VideoMedia(DEMO_VIDEO_SRC)],
        ...
        # Fallback: if ft.Video is not available in this Flet build,
        # show a styled placeholder with instructions.
        video_widget = ft.Container(
            content=ft.Column([
                ft.Text("▶", size=36, color=color, text_align=ft.TextAlign.CENTER),
                ft.Text(
                    "Video player requires Flet ≥ 0.21\n"
                    f"File: assets/{DEMO_VIDEO_SRC}",
    ...
    screenshot_card = card(ft.Column([
        ...
        ft.GestureDetector(
            ...
            content=ft.Stack([
                ft.Container(
                    content=ft.Image(
                        src="mubita-portfolio/assets/github/git.png",
